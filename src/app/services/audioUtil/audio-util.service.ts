...
export class AudioUtilService {
  ...
  //Private methods to do the Interactions
  ...
  mergeBuffers(recBuffers, recLength) {
    ...
    let offset = 0;
    for (let counter = 0; counter < recBuffers.length; counter++) {
      result.set(recBuffers[counter], offset);
      offset += recBuffers[counter].length;
    }
    return result;
  }
